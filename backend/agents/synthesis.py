...
USER QUERY: {query}"""
    ...
    def _format_textbook_context(self, results: List[Dict]) -> str:
        ...
        for i, result in enumerate(results[:5], 1):
            page_info = f", Page {result['page_num']}" if result.get('page_num') else ""
            context += f"\n{i}. From '{result['book_title']}'{page_info}:\n"
